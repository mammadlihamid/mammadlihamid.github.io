...
const parseStatus = data => {
  return {
    text: data.discord_status.replace('online', 'Onlayn').replace('idle', 'Bekar').replace('dnd', 'Narahat etməyin').replace('offline', 'Oflayn'),
...
const parseActivity = (data, activityData) => {
  return activityData !== undefined ? `"${data.spotify.song}" Qulaq asır <a class="spotify-time">${activityData.current} ─ ${activityData.end}</a>` : 'Hal-hazırda qulaq asmır.';
};

const reloadUser = async (data) => {
  clearInterval(currentInterval);
  $('div-img.avatar').css('background-image', `url(https://cdn.discordapp.com/avatars/${discordId}/${data.discord_user.avatar}.jpeg?size=512)`).removeAttr('loading');
  ...
  ${data.spotify.artist.replace(/;/g, ',')} tərəfindən`);
    activityTimer(data);
  } else {
...
function showGithubRepos() {
  const getGitHubData = async () => {
    const response = await fetch(properties.github_api.baseURL + '/users/mammadlihamid/repos');
