...
const discordId = "821760729191415829";
...
const parseStatus = (data) => {
  return {
    text: data.discord_status
      .replace("online", "Online")
      .replace("idle", "Idle")
      .replace("dnd", "Do not disturb")
      .replace("offline", "Offline"),
    color: data.discord_status
      .replace("online", "rgb(69 189 69)")
      .replace("idle", "#ffa500")
      .replace("dnd", "rgb(202, 71, 71)")
      .replace("offline", "var(--secondary-color)"),
  };
};
...
const reloadUser = async (data) => {
  ...
  $("div-img.avatar")
    .css(
      "background-image",
      `url(https://cdn.discordapp.com/avatars/${discordId}/${data.discord_user.avatar}.png?size=512)`
    )
    .removeAttr("loading");
  $("status > p")
    .text(parseStatus(data).text)
    .removeAttr("loading")
    .css("width", "max-content");
  $("status > span").css("background-color", parseStatus(data).color);
  $("status")
    .attr("tippy-interactive", "true")
    .attr(
      "tippy",
      `
  ...
  <a class="discord" href="https://discord.com/users/821760729191415829" target="_blank">Discord</a>`
    );
  ...
  ${data.spotify.artist.replace(/;/g, ",")} by`
    );
    activityTimer(data);
  } else {
    $("spotify > logo")
  ...
  $("spotify > p")
    .html(parseActivity(data, activityData))
    .removeAttr("loading")
    .css("width", "max-content");
...
const changeBlock = (id) => {
  ...
  document.querySelector("block-content").innerHTML =
    block.querySelector("extent").innerHTML;
...
function showGithubRepos() {
  const getGitHubData = async () => {
    const response = await fetch(
      properties.github_api.baseURL + "/users/mammadlihamid/repos"
    );
  ...
  getGitHubData().then((data) => {
    data.forEach((repository, index) => {
      ...
      repositoryElement.onclick = () =>
        window.open(repository.html_url, "_blank");
      repositoryElement.innerHTML = `<repo-title>${
...
function loadIcons() {
  ...
  const socials = [
    {
      ...
      href: "https://www.instagram.com/azveoz001/",
      tippy: "Instagram",
    },
    {
      ...
      href: "mailto:hemid359@gmail.com",
      tippy: "E-Mail",
    },
    {
      ...
      href: `https://discord.com/users/821760729191415829`,
      tippy: "Discord",
    },
    {
      ...
      href: "https://github.com/mammadlihamid",
      tippy: "GitHub",
    },
  ];
  ...
  $("links > a").html(
    socials.map(
      (social) =>
        `<a href="${social.href}" target="_blank" rel="noopener noreferrer"><span tippy="${social.tippy}">${social.svg}</span>`
    )
  );
  reloadTippys();
}
...
function activityTimer(data) {
  ...
  var interval = setInterval(async () => {
    ...
    if (
      ...
      Math.floor(data.spotify.timestamps.end / 1000)
    ) {
      const newData = reloadUser(data);
      if (!newData?.listening_to_spotify)
        $("spotify > logo")
          .html(`<svg xmlns="http://www.w3.org/2000/svg" height="168px" width="168px" version="1.1" viewBox="0 0 168 168">
      <path fill="green" d="m83.996 0.277c-46.249 0-83.743 37.493-83.743 83.742 0 46.251 37.494 83.741 83.743 83.741 46.254 0 83.744-37.49 83.744-83.741 0-46.246-37.49-83.738-83.745-83.738l0.001-0.004zm38.404 120.78c-1.5 2.46-4.72 3.24-7.18 1.73-19.662-12.01-44.414-14.73-73.564-8.07-2.809 0.64-5.609-1.12-6.249-3.93-0.643-2.81 1.11-5.61 3.926-6.25 31.9-7.291 59.263-4.15 81.337 9.34 2.46 1.51 3.24 4.72 1.73 7.18zm10.25-22.805c-1.89 3.075-5.91 4.045-8.98 2.155-22.51-13.839-56.823-17.846-83.448-9.764-3.453 1.043-7.1-0.903-8.148-4.35-1.04-3.453 0.907-7.093 4.354-8.143 30.413-9.228 68.222-4.758 94.072 11.127 3.07 1.89 4.04 5.91 2.15 8.976v-0.001zm0.88-23.744c-26.99-16.031-71.52-17.505-97.289-9.684-4.138 1.255-8.514-1.081-9.768-5.219-1.254-4.14 1.08-8.513 5.221-9.771 29.581-8.98 78.756-7.245 109.83 11.202 3.73 2.209 4.95 7.016 2.74 10.733-2.2 3.722-7.02 4.949-10.73 2.739z"/>
      </svg>`);
    } else
      document.querySelector("spotify > p").innerHTML = await parseActivity(
        data,
        activityData
      );
  }, 1000);
...
document.body.onload = () => {
  document.querySelector("block-content").innerHTML = document
    .querySelector("blocks > div.active")
    .querySelector("extent").innerHTML;
  if (document.querySelector("blocks > div.active").getAttribute("id") == "1")
    showGithubRepos();
};
...
const getActivityData = async (userData) => {
  if (userData.listening_to_spotify === true) {
    ...
    let currentDateSecs =
      Math.floor((currentSeconds / 1000) % 60) < 10
        ? "0" + Math.floor((currentSeconds / 1000) % 60)
        : Math.floor((currentSeconds / 1000) % 60);
    let currentDate = `${Math.floor(
      (currentSeconds / (1000 * 60)) % 60
    )}:${currentDateSecs}`;
    ...
    let endDateSecs =
      Math.floor(
        ...
        ? "0" +
          Math.floor(
            ((userData.spotify.timestamps.end -
              userData.spotify.timestamps.start) /
              1000) %
              60
          )
        : Math.floor(
